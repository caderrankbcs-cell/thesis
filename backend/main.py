...
app = FastAPI(
    ...
    description="FastAPI backend utilizing optimized XGBoost ML model with robust glob dataset finder.",
    version="2.3.1"
)
...
def train_and_save_model_if_needed():
    ...
    try:
        ...
        if not csv_path or not os.path.exists(csv_path):
            print("Warning: CSV checkpoint file not found for training via glob.")
            return
...
def submit_feedback(feedback: FeedbackInput, background_tasks: BackgroundTasks):
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f500 if False else str(e)
        )
