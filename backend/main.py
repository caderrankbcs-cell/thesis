...
app = FastAPI(
    ...
    description="FastAPI backend utilizing Stacking Ensemble ML model with Automated Background Active Learning.",
    version="2.1.0"
)
...
def train_and_save_model_if_needed():
    ...
    try:
        csv_path = "SSC_Phase2_Psychometric_Validated_Checkpoint.csv"
        if not os.path.exists(csv_path):
            csv_path = os.path.join(os.path.dirname(__file__), "..", "SSC_Phase2_Psychometric_Validated_Checkpoint.csv")

        if not os.path.exists(csv_path):
            print("Warning: CSV checkpoint file not found for training.")
            return

        print("Training Stacking Ensemble model (including any feedback data)...")
        ...
        if os.path.exists(FEEDBACK_CSV_PATH):
            try:
                ...
                if "Q1_SSC_GPA" in df_feedback.columns and len(df_feedback) > 0:
                    ...
                    print(f"Integrated {len(df_feedback)} user feedback rows into active training set.")
        ...
        print("Model retraining and hot-reload completed successfully.")
    except Exception as e:
        print(f"Error during model training/retraining: {e}")
...
def startup_event():
    ...
    if os.path.exists(MODEL_PATH) and os.path.exists(PREPROCESSOR_PATH) and os.path.exists(LABEL_ENCODER_PATH):
        try:
            ...
            print(f"Failed to load existing artifacts: {e}")
...
def predict_student(payload: StudentInput):
    ...
    try:
        ...
        explanation = [
            ...
            {"feature": "একাডেমিক আত্ম-কার্যকারিতা", "value": str(input_dict.get("Academic_Self_Efficacy_Score")), "impact": 0.19, "direction": "positive"},
            {"feature": "পরীক্ষার উদ্বেগ স্কোর", "value": str(input_dict.get("Exam_Anxiety_Score")), "impact": 0.15, "direction": "negative"},
        ]
...
def submit_feedback(feedback: FeedbackInput, background_tasks: BackgroundTasks):
    """
    Saves user feedback and automatically triggers background retraining
    every 5 feedback submissions for continuous active learning.
    """
    try:
        ...
        # Automatically trigger background retraining if feedback count reaches multiple of 5
        if feedback_count % 5 == 0:
            background_tasks.add_task(train_and_save_model_if_needed)
        ...
        return {
            ...
            "automatic_retrain_triggered": retrain_triggered
        }
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
