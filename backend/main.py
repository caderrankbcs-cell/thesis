...
app = FastAPI(
    ...
    description="FastAPI backend utilizing optimized XGBoost ML model for reliable Cloud deployment.",
    version="2.2.0"
)
...
def train_and_save_model_if_needed():
    ...
    try:
        ...
        print(f"Training XGBoost model using dataset: {csv_path}...")
        ...
        class_weights = {"3.00 - 4.49": 0.494, "4.50 - 5.00": 1.167, "Below 3.00": 8.458}
        sample_weights = np.array([class_weights[le.inverse_transform([i])[0]] for i in y_encoded])
        ...
        clf.fit(X_encoded, y_encoded, sample_weight=sample_weights)
...
def predict_student(payload: StudentInput):
    ...
    try:
        ...
        explanation = [
            ...
            {"feature": "Exam Anxiety Score", "value": str(input_dict.get("Exam_Anxiety_Server", input_dict.get("Exam_Anxiety_Score"))), "impact": 0.15, "direction": "negative"},
        ]
